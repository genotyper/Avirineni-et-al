...
#drop the WI group per reviewer
pd = subset_samples(pd, group != "Whey+Inulin")
temp = data.frame(otu_table(pd))
drop = names(which(rowSums(temp) != 0))
pd = prune_taxa(drop, pd)
met = data.frame(sample_data(pd))
tax = data.frame(tax_table(pd))
...
# ord = unique(sample_data(pd)$group)
sample_data(pd)$group = ordered(sample_data(pd)$group, levels = c("Egg+Cellulose", "Egg+Inulin", "Whey+Cellulose", "Control"))
out = ancombc(phyloseq = pd, formula = "group", lib_cut = 1)
res = out$res
sig = c()
for(i in 1:2){
...
ec_ref = res$q_val[sig,1:2]
names(ec_ref)[1:2] = c("Egg+Inulin", "Whey+Cellulose")

sample_data(pd)$group = ordered(sample_data(pd)$group, levels =c("Egg+Inulin", "Whey+Cellulose", "Control", "Egg+Cellulose"))
out = ancombc(phyloseq = pd, formula = "group", lib_cut = 1)
res = out$res
sig = c()
for(i in 1){
...
ei_ref = res$q_val[sig,1, drop = FALSE]
names(ei_ref)[1] = "Whey+Cellulose"
...
pw = rbind(con_ref, ec_ref, ei_ref)
pw$z = sub("\\..*", "", pw$z)
...
library(pheatmap)
# png('r2figs/heatmap.png', res = 600, width = 9, height = 18, units = 'in')
...
# png('r2figs/heatmap2.png', res = 600, width = 9, height = 10, units = 'in')
...
temp = temp[,c(12,11,10,9,3,1,2,4,5,6,7,8)]
temp = temp[order(temp$q_value),]
# write.csv(temp, "r2figs/da.csv")

#see if higher taxonomies are disproportionately on the signficance list
look = merge(data.frame(table(t2$Phylum)), data.frame(table(temp$Phylum)), by = "Var1", all = TRUE)
names(look) = c("rank", "total", "sig")
look[is.na(look)] = 0
bigtest = c()
for(i in 1:nrow(look)){
  more = binom.test(x = look$sig[i], n = (look$total[i]*6), look$total[i]/381, alternative = "greater")$p.value
  less = binom.test(x = look$sig[i], n = (look$total[i]*6), look$total[i]/381, alternative = "less")$p.value
  test = data.frame(phylum = look$rank[i], more = more, less = less)
  bigtest = rbind(bigtest, test)
}
...
dd$total_species = c(218, 258, 272)
dd$Distance = dd$Freq/dd$total_species
dd$Var1 = sub(".*group", "", dd$Var1)
dd$Var1 = ordered(dd$Var1, levels = dd$Var1)
p = ggplot(dd, aes(Var1, Distance)) + geom_point(size = 5, color = "grey60") + geom_point(size = 2) + ylim(0,0.5) + labs(x = "", y = "Distance from Control\n(Proportion of Species DA)") + theme_bw(base_size = 15)
# ggsave("r2figs/DA_from_control.png", p, height = 4, width = 6, dpi = 300, units = "in")
...
# ggsave("r2figs/bar.png", p, width = 16.75, height = 8, units = "in", dpi = 600)
p2 = p
#####
#beta
#####
...
holda = a
# write.csv(data.frame(a[[1]]), "r2figs/adonis.csv")
#####
...
#####
#correlate metabolomics with species and genera
#####
...
keep = unique(keep)
length(unique(keep)) #how many were above 2%
...
length(unique(annotation_col$Phylum)) #how many phyla represented

cclass = read.csv("chemical_classes_v2.csv")
row.names(cclass) = cclass$chemical
cclass = cclass[,1,drop = FALSE]

table(abs(t1) > 0.1)[2]/sum(table(abs(t1) > 0.1))

# png('r2figs/species_metabalomics_heatmap_phylum.png', res = 600, width = 10, height = 17, units = 'in')
...
#genus-level
t1 = data.frame(t(chem))
d = tax_glom(pd, taxrank = "Genus")
t2 = data.frame(t(data.frame(otu_table(d))))
t2 = t2/rowSums(t2)
t2 = data.frame(t(t2))
t2 = t2[,names(t2) %in% names(t1)]
keep = c()
...
t1 = t1[,colnames(t1) %in% row.names(t2)]
t1 = t1[!rownames(t1) %in% row.names(t2),]
temp = data.frame(tax_table(d))
colnames(t1) = temp[,6][match(colnames(t1), row.names(temp))]
m = melt(t1)
...
# png('r2figs/genus_metabalomics_heatmap.png', res = 600, width = 8, height = 17, units = 'in')
# print(pheatmap((t1),fontsize_col = 10, fontsize_row = 10, display_numbers = FALSE, labels_col  = as.expression(newnames)))
# dev.off()

####
#read in microbiome function
#####
f = read.delim("prasanth_otus_Ref99NR_95/pathway_prediction.txt", row.names = 1)
...
#this drops any WI samples
f = f[row.names(f) %in% row.names(met),]
chem = chem[row.names(chem) %in% row.names(met),]
...
# write.csv(data.frame(a[[1]]), "r2figs/function_adonis.csv")

both = data.frame(set = c(rep("Composition", 3), rep("Function", 3)), variance = rep(row.names(a[[1]])[1:3],2), r2 = c(holda[[1]][1:3,5], a[[1]][1:3,5]))
...
p$data$variance = factor(p$data$variance, ordered = TRUE, levels = c("protein", "fiber", "interaction", "residual"))
# ggsave("r2figs/adonis_bar.png", p, width = 5, height = 6, units = "in", dpi = 600)
...
#sig of correlations and binom test for metabolite enrichment
sigs = c()
for(i in 1:ncol(chem)){
  for(j in 1:ncol(f)){
    temp = data.frame(chem = names(chem)[i], func = names(f)[j], p.value = cor.test(t1[,i], t2[,j], method = "spearman")$p.value)
    sigs = rbind(sigs, temp)
  }
  print(i)
}
table(sigs$p.value <= 0.05)
2347/40479 #5.8%
bt = as.data.frame(sort(table(sigs$chem[which(sigs$p.value <= 0.05)])))
#bionmial test to see if certain metabolites are disproportionally showing up as significant

bt$binomial = 1
for(i in 1:nrow(bt)){
  bt$binomial[i] = binom.test(bt$Freq[i], 2347, ((1/133) * 2347/2347), alternative = "greater")$p.value
}

#how many metabolites were sig at least ones
length(unique(sigs$func[which(sigs$p.value <= 0.05)]))

names(bt) = c("metabolite", "significant.associations", "binomial.test.p.value")
write.csv(bt, "r2figs/supS7.csv", row.names = F)

# dc$func = row.names(dc)
sigs = merge(sigs, dc, "func")
sigs = sigs[,c(3,2,1,4,5,6)]
write.csv(sigs, "r2figs/supS6.csv", row.names = F)
...
# png('r2figs/noclustmbfunction_metabalomics_heatmap.png', res = 600, width = 15, height = 22, units = 'in')
...
# png('r2figs/clustmbfunction_metabalomics_heatmap.png', res = 600, width = 15, height = 22, units = 'in')
...
row.names(annotation_row) = row.names(dc)
# png('r2figs/l3_clustmbfunction_metabalomics_heatmap.png', res = 600, width = 15, height = 22, units = 'in')
...
for(i in 1:length(pc)){print(dim(pc[[i]]))}
possig = c()
...
ggsave("r2figs/rda_subsets.png", p, height = 4, width = 6, dpi = 300, units = "in")
pxx = p
keep = unique(m$Var2[which(m$value <= 0.56)])
f_sub = f[,names(f) %in% keep, drop = F]
...
#####
#this would create input for an LEfSe analysis on Galaxy if desired
#####
f = data.frame(otu_table(pd))
f = f/rowSums(f)
t = data.frame(tax_table(pd))
row.names(f) = paste(t$Kingdom, t$Phylum, t$Class, t$Order, t$Family, t$Genus, t$Species, sep = "|")
f = data.frame(t(f))
f$class = met$group
f$id = row.names(f)
f = f[,c((ncol(f) - 1), ncol(f), 1:(ncol(f) - 2))]
f = data.frame(t(f))
write.table(f, "LEfSe_input.tab",  quote = F, row.names = T, col.names = F, sep = "\t")
